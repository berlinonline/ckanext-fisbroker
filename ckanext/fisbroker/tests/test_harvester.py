...
from ckanext.fisbroker.tests import FisbrokerTestBase, base_context, WFS_FIXTURE, FISBROKER_PLUGIN
...
class TestHarvester(FisbrokerTestBase):
    ...
    def test_empty_config(self):
        ...
        assert FisbrokerHarvester().validate_config(None) == None
        assert FisbrokerHarvester().validate_config({}) == {}
    ...
    def test_import_since_must_be_valid_iso(self):
        ...
        assert FisbrokerHarvester().validate_config(config)
        # invalid date:
        config = '{ "import_since": "2019.01.01" }'
    ...
    def test_undefined_import_since_is_none(self):
        ...
        FisbrokerHarvester().source_config = {}
        import_since = FisbrokerHarvester().get_import_since_date(None)
    ...
    def test_import_since_big_bang_means_none(self):
        ...
        FisbrokerHarvester().source_config = { 'import_since': "big_bang" }
        import_since = FisbrokerHarvester().get_import_since_date(None)
        assert import_since == None
    ...
    def test_import_since_regular_value_returned_unchanged(self):
        ...
        FisbrokerHarvester().source_config = {'import_since': "2020-03-01"}
        import_since = FisbrokerHarvester().get_import_since_date(None)
    ...
    def test_undefined_timeout_gives_default(self):
        ...
        FisbrokerHarvester().source_config = {}
        timeout = FisbrokerHarvester().get_timeout()
    ...
    def test_undefined_time_delta_gives_default(self):
        ...
        FisbrokerHarvester().source_config = {}
        timedelta = FisbrokerHarvester().get_timedelta()
    ...
    def test_timeout_config_returned_as_int(self):
        ...
        FisbrokerHarvester().source_config = { 'timeout': '100' }
        timeout = FisbrokerHarvester().get_timeout()
        assert timeout == 100
    ...
    def test_timedelta_config_returned_as_int(self):
        ...
        FisbrokerHarvester().source_config = { 'timedelta': '1' }
        timedelta = FisbrokerHarvester().get_timedelta()
        assert timedelta == 1
    ...
    def test_last_error_free_returns_correct_job(self, app, base_context):
        ...
        FisbrokerHarvester().source_config['import_since'] = "last_error_free"
        import_since = FisbrokerHarvester().get_import_since_date(new_job)
        import_since_expected = (job.gather_started +
                                 timedelta(hours=FisbrokerHarvester().get_timedelta()))
        ...
        constraint = FisbrokerHarvester().get_constraints(new_job)[0]
    ...
    def test_last_error_free_does_not_return_unsuccessful_job(self, base_context):
        ...
        object_ids = gather_stage(FisbrokerHarvester(), job_a)
        for object_id in object_ids:
            harvest_object = HarvestObject.get(object_id)
            fetch_and_import_stages(FisbrokerHarvester(), harvest_object)
        ...
        object_ids = gather_stage(FisbrokerHarvester(), job_b)
        for object_id in object_ids:
            harvest_object = HarvestObject.get(object_id)
            fetch_and_import_stages(FisbrokerHarvester(), harvest_object)
    ...
    def test_import_since_date_is_none_if_no_jobs(self, base_context):
        ...
        source, job = self._create_source_and_job()
        FisbrokerHarvester().source_config['import_since'] = "last_error_free"
        import_since = FisbrokerHarvester().get_import_since_date(job)
        assert import_since == None
